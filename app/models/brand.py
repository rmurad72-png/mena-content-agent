...
from sqlalchemy import Boolean, CheckConstraint, Enum as SAEnum, ForeignKey, ForeignKeyConstraint, Index, String, Text, UniqueConstraint
...
class Brand(UUIDPrimaryKeyMixin, TimestampMixin, SoftDeleteMixin, Base):
    ...
    primary_language: Mapped[str] = mapped_column(String(10), nullable=False, default="ar", server_default="ar")
    status: Mapped[BrandStatus] = mapped_column(SAEnum(BrandStatus, name="brand_status"), nullable=False, default=BrandStatus.active, server_default="active")
    __table_args__ = (
        ...
        CheckConstraint("primary_language IN ('ar','en')", name="brand_language_allowed"),
        Index("ix_brands_workspace_status", "workspace_id", "status"),
    )


class BrandIdentity(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (UniqueConstraint("brand_id", name="uq_brand_identities_brand"),)


class BrandVoice(UUIDPrimaryKeyMixin, TimestampMixin, Base):
...
class BrandAudience(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("language IN ('ar','en')", name="brand_audience_language_allowed"),
        Index("ix_brand_audiences_brand_language", "brand_id", "language"),
    )


class BrandRule(UUIDPrimaryKeyMixin, TimestampMixin, SoftDeleteMixin, Base):
    ...
    name: Mapped[str] = mapped_column(String(150), nullable=False)
    description: Mapped[str | None] = mapped_column(Text)
    severity: Mapped[BrandRuleSeverity] = mapped_column(SAEnum(BrandRuleSeverity, name="brand_rule_severity"), nullable=False, default=BrandRuleSeverity.warning, server_default="warning")
    rule_config: Mapped[dict] = mapped_column(JSONB, nullable=False, default=dict, server_default="{}")
    is_active: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True, server_default="true")
    __table_args__ = (Index("ix_brand_rules_brand_type_active", "brand_id", "rule_type", "is_active"),)


class BrandTerm(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("language IN ('ar','en')", name="brand_term_language_allowed"),
        CheckConstraint("length(trim(term)) > 0", name="brand_term_not_blank"),
        CheckConstraint("length(trim(preferred_form)) > 0", name="brand_term_preferred_not_blank"),
        Index("ix_brand_terms_brand_language", "brand_id", "language"),
    )


class BrandMemberRole(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    role_scope: Mapped[RoleScope] = mapped_column(SAEnum(RoleScope, name="role_scope"), nullable=False, default=RoleScope.brand, server_default="brand")
    __table_args__ = (
        ...
        UniqueConstraint("workspace_member_id", "brand_id", "role_id", name="uq_brand_member_roles_member_brand_role"),
